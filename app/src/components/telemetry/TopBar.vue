...
<template>
  <header class="flex items-center justify-between px-4 h-10 bg-[var(--bg-panel)] border-b border-[var(--bd-dim)] shrink-0">
    <div class="flex items-center gap-1.5">
      <span
        class="w-2 h-2 rounded-full transition-colors duration-300"
        :class="connected ? 'bg-green-500 shadow-[0_0_6px_#22c55e]' : 'bg-red-500'"
      />
      <span class="text-[0.7rem] font-bold tracking-wider text-[var(--tx-dim)]">
        {{ connected ? '实时遥测' : '等待连接…' }}
      </span>
    </div>
    ...
    <div class="flex items-center gap-1 min-w-0 overflow-hidden">
      <span
        class="text-[clamp(0.7rem,1.6vw,0.85rem)] font-semibold text-[var(--tx-mid)] whitespace-nowrap overflow-hidden text-ellipsis max-w-[clamp(80px,22vw,260px)]"
      ...
      <span
        class="text-[clamp(0.55rem,1.2vw,0.65rem)] font-bold px-1 py-[0.1rem] border rounded flex-shrink-0 whitespace-nowrap"
        :class="classBadgeColors[classLabel] ?? 'text-[var(--tx-lo)] border-[var(--bd-muted)]'"
      >
        {{ classLabel }}
      </span>
      <span class="text-[clamp(0.55rem,1.2vw,0.65rem)] font-bold px-1 py-[0.1rem] border border-[var(--bd-muted)] rounded text-[var(--tx-lo)] flex-shrink-0 whitespace-nowrap">
        {{ piLabel }}
      </span>
      <span class="text-[clamp(0.55rem,1.2vw,0.65rem)] font-bold px-1 py-[0.1rem] border border-[var(--bd-muted)] rounded text-[var(--tx-lo)] flex-shrink-0 whitespace-nowrap">
        {{ driveLabel }}
    ...
    <div class="flex items-center gap-1">
      <button
        class="text-[0.5rem] font-bold tracking-wider px-1 py-[0.15rem] border rounded cursor-pointer transition-colors"
        :class="tiresVisible
      ...
      >轮胎</button>
      <button class="bg-none border-none cursor-pointer text-[1rem] text-[var(--tx-dim)] px-2 py-1 rounded hover:bg-[var(--bg-elevated)] hover:text-[var(--tx-mid)]" @click="onSessions?.()" title="历史会话">⏱</button>
      <button class="bg-none border-none cursor-pointer text-[1rem] text-[var(--tx-dim)] px-2 py-1 rounded hover:bg-[var(--bg-elevated)] hover:text-[var(--tx-mid)]" @click="onSettings?.()" title="设置">⚙</button>
      <span v-if="version" class="text-[0.6rem] text-[var(--tx-xdim)] tracking-wide px-[0.1rem]">v{{ version }}</span>
    </div>
  </header>
</template>