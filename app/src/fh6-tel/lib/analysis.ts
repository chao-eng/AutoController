...
export function buildLapChips(segments: LapSegment[], laps: SessionLap[]): LapChip[] {
  ...
  return segments.map((segment) => {
    ...
    return {
      ...
      label: `第 ${segment.lapNumber + 1} 圈${occ > 1 ? ` (${occ})` : ''}`,
      lapTime,
...
export function metricGroups(useMph: boolean): MetricGroup[] {
  ...
  const speedLabel = useMph ? '车速 (mph)' : '车速 (km/h)';
  return [
    {
      title: '驾驶操作输入',
      metrics: [
        { label: '油门 %', dash: [], value: (p) => (p.throttle / 255) * 100 },
        { label: '刹车 %', dash: [6, 4], value: (p) => (p.brake / 255) * 100 },
        { label: '离合 %', dash: [2, 3], value: (p) => (p.clutch / 255) * 100 },
      ],
    },
    {
      title: '速度与转速',
    ...
    },
    {
      title: '加速度 G 值',
      metrics: [
        { label: '横向 G 值', dash: [], value: (p) => p.accelX / 9.80665 },
        { label: '纵向 G 值', dash: [6, 4], value: (p) => p.accelZ / 9.80665 },
      ],
    },
    {
      title: '轮胎温度 (°C)',
      metrics: [
        { label: '前左', dash: [], value: (p) => p.tireTempFl },
        { label: '前右', dash: [6, 4], value: (p) => p.tireTempFr },
        { label: '后左', dash: [2, 3], value: (p) => p.tireTempRl },
        { label: '后右', dash: [8, 3, 2, 3], value: (p) => p.tireTempRr },
      ],
    },
  ];
}
...
export function buildChart(group: MetricGroup, selected: LapChip[]): AlignedChart {
  ...
  for (const chip of selected) {
    ...
    for (const metric of group.metrics) {
      ...
      series.push({
        label: `第${chip.lapNumber + 1}圈 ${metric.label}`,
        stroke: chip.color,
        dash: metric.dash,
        data,
      });
