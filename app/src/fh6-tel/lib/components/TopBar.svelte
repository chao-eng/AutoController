<script lang="ts">
  ...
  let { useMph = true, onSettings, onSessions, tiresVisible = true, onToggleTires }: {
    useMph: boolean;
    onSettings: () => void;
    onSessions: () => void;
...
<header class="topbar">
  <div class="status">
    ...
    <span class="label">{connected ? '实时遥测' : '等待连接…'}</span>
  </div>
  ...
  <div class="controls">
    ...
    >轮胎</button>
    <button class="icon-btn" onclick={onSessions} title="历史会话">⏱</button>
    <button class="icon-btn" onclick={onSettings} title="设置">⚙</button>
