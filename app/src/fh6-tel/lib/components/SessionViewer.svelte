<script lang="ts">
  import { loadSessionPackets, loadSessionLaps, renameSession, setSessionBookmark } from '$lib/stores/sessions';
...
<div class="overlay" role="dialog" aria-modal="true">
  <div class="viewer">
    <header>
      <div class="title-area">
        {#if editing}
          ...
          />
        {:else}
          <button class="name-display" onclick={startEdit} title="点击重命名">
            {displayName}
    ...
    <div class="content">
      {#if loading}
        <p class="status">正在加载 {session.packetCount} 个数据包…</p>
      {:else if packets.length === 0}
        <p class="status">此会话没有记录遥测数据。</p>
      ...
      {:else}
        <div class="replay-panel">
          ...
          <button class="replay-go" onclick={beginReplay}>▶ 在仪表盘上回放</button>
        </div>
      {/if}
    </div>
  </div>
