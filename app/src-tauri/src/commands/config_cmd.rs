...
pub fn config_set(
    ...
    if old_level != new_level {
        if let Ok(new_filter) =
            tracing_subscriber::EnvFilter::try_new(format!("{},tao=error", new_level))
        {
                        ...
                        if let (Ok(x), Ok(y), Ok(w), Ok(h)) = (
                            ...
                            if vec_idx < config.ocr_regions.len() {
                                ...
                                while config.ocr_regions.len() < vec_idx {
                                    // 填充占位符区域，保证数组连续性
                                    config.ocr_regions.push(crate::config::OcrRegion {
                                        x: 0,
                                        y: 0,
                                        w: 0,
                                        h: 0,
...
pub fn import_backup_data(
    ...
    if let Some(ref val) = backup.macros {
        data_dir
            .save("macros", val)
            .map_err(|e| format!("无法保存 macros 配置: {}", e))?;
    }
    ...
    if let Some(ref val) = backup.scripts {
        data_dir
            .save("scripts", val)
            .map_err(|e| format!("无法保存 scripts 配置: {}", e))?;
    }
    ...
    if let Some(ref val) = backup.tasks {
        data_dir
            .save("tasks", val)
            .map_err(|e| format!("无法保存 tasks 配置: {}", e))?;
    }
    ...
    if let Some(ref val) = backup.config {
        ...
        data_dir
            .save("config", val)
            .map_err(|e| format!("无法保存 config 配置: {}", e))?;
