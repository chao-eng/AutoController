...
#### Forza 遥测页面整体 UI 体验优化

- **历史会话抽屉视觉收敛**：优化右侧历史会话抽屉的标题栏交互层级，去除组件默认关闭按钮与自定义关闭按钮并存造成的重复控制，让抽屉顶部操作区更加清爽。
- **会话详情弹框宽度重新校准**：调整统计分析 / 数据回放弹框的响应式宽度，避免默认小弹框宽度导致图表拥挤，同时控制桌面端最大宽度不过度铺满屏幕，在图表可读性和界面聚焦感之间取得更稳定的平衡。
- **遥测分析阅读体验打磨**：针对历史会话查看、圈速分析、驾驶输入曲线和速度 / 转速图表等高频查看路径，改善弹层空间分配，让用户在回看数据时减少横向压缩造成的信息拥挤。

### 🐛 Bug 修复

- **历史会话抽屉关闭按钮去重**：修复 Forza 遥测页面历史会话右侧抽屉顶部同时出现两个关闭按钮的问题，保留标题栏内的关闭按钮，减少视觉干扰。
