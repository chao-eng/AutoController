...
- **PaddleOCR Rust 原生绑定与完全本地化内置 (app/src-tauri/src/ocr)：**将 PaddleOCR 核心识别引擎通过 Rust 底层原生绑定（Binding）直接无缝嵌入主程序中，拒绝臃肿的 Python 环境或外部进程调用。配合内置的轻量化预测模型，实现全离线、零网络依赖的图文识别。
